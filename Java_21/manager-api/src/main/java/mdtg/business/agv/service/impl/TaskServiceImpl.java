...
public class TaskServiceImpl extends ServiceImpl<TaskMapper, Task> implements TaskService {
    ...
    public ResponseDTO<?> queryTask(QueryTaskInputDTO inputDTO) {
        ...
        Optional.ofNullable(inputDTO.getName()).ifPresent(name -> queryWrapper.eq(Task::getName, name));
        ...
        Optional.ofNullable(inputDTO.getRemark()).ifPresent(remark -> queryWrapper.eq(Task::getRemark, remark));
        IPage<Task> page = new Page<>(inputDTO.getPageNum(), inputDTO.getPageSize());
        IPage<Task> taskIPage = this.baseMapper.selectPage(page, queryWrapper);
        return ResponseDTO.wrapSuccess(taskIPage);
    }
}
