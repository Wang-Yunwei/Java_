...
CREATE TABLE `mdtg_user` (
    ...
    `sys_user_id`       BIGINT          NOT NULL        COMMENT 'sys_user表主键ID',
...
CREATE TABLE `mdtg_role` (
    `id`                BIGINT          NOT NULL        COMMENT '主键',
    `code`              VARCHAR(50)     DEFAULT NULL    COMMENT '编码(如: admin)',
...
CREATE TABLE `mdtg_task` (
    ...
    `type`                  TINYINT         DEFAULT 0       COMMENT '类型(0-手动,1-定时)',
    `status`                TINYINT         DEFAULT 0       COMMENT '状态(0-未执行,1-执行中,2-已完成,3-执行失败)',
    `cron_expression`       VARCHAR(20)     DEFAULT NULL    COMMENT 'Cron 表达式: 0 0 18 ? * WED',
...
(
    ...
    `voice_id`    BIGINT       NOT NULL COMMENT '声音ID',
